...
export const getTitle = ($, data) => {
    ...
    data.info.onAir = (data.info.onAir) ? data.info.onAir.split("–") : null
    data.info.onAir = (data.info.onAir) ? { start : parseInt(data.info.onAir[0]), end: parseInt(data.info.onAir?.[1]) } : null
    ...
    data.people = { directors : null, stars : null, cast : null }
    ...
    data.seasons = $('[data-testid="episodes-browse-episodes"] a').map( (item, elem) => {
        const x = $(elem).text()?.toLowerCase()
        if(!x?.includes("season")) return;
        return parseInt(x.split(/\b/)[0])
    }).get()?.[0]
