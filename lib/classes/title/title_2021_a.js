...
export default class Title {
    ...
    constructor ($, data) {
        ...
        data.info.id = $('[property="imdb:pageConst"]').attr('content');
        ...
        data.info.onAir = $(header[1]).find("span").last().text() 
        ...
        data.info.plot = (data.plot == "Add a plot") ? null : data.plot;
        ...
        data.info.type = $('[property="og:type"]').attr('content')?.replace('video.', '');
        ...
        data.info.links.officialSite = $('[data-testid="title-details-officialsites"] li').map(function () { return $(this).text().trim()}).toArray();
        ...
        data.crew.directors = $('li[data-testid="title-pc-principal-credit"]:first-child li').map(function () { 
            return { name : $(this).text().trim(), id: getID($(this).find("a").attr('href'))}
        }).toArray().filter(makeUnique);

        data.crew.stars = $('li[data-testid="title-pc-principal-credit"]:last-child li').map(function () { 
            return { name : $(this).text().trim(), id: getID($(this).find("a").attr('href'))}
        }).toArray().filter(makeUnique);
